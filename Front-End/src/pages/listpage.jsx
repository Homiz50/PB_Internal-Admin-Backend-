...
export default function PropertyTable() {
  ...
  <button className="bg-purple-600 text-white px-3 py-1 rounded-lg text-xl transition-transform duration-300 hover:scale-110">
    Logout
      ...
      <p className="text-white m-3 font-bold text-2xl text-center p-3 w-1/3 mx-auto rounded-lg animate-bgColorChange">
              ...
              <tr key={property._id} className="text-center border text-xs">
                <td className={` border ${isFieldMissing(property.data.date) ? '' : ''}`}>
                  <input
                    ...
                    className="w-20 text-center bg-transparent border-none outline-none font-medium"
                  />
                </td>
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.price) ? '' : ''}`}>
                  <input
                    type="text"
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.bhk) ? '' : ''}`}>
                  <input
                    type="text"
                    value={property.data.bhk || ''}
                    onChange={(e) => handleFieldChange(property._id, 'bhk', e.target.value)}
                    className="w-auto text-center bg-transparent border-none outline-none font-medium"
                  />
                </td>
                <td className={`p-2 border ${isFieldMissing(property.data.squr) ? '' : ''}`}>
                  <input
                    type="text"
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.project_name) ? '' : ''}`}>
                  <input
                    type="text"
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.address) ? '' : ''}`}>
                   <textarea
                      value={property.data.address || ''}
                      onChange={(e) => handleFieldChange(property._id, 'address', e.target.value)}
                      className="w-auto bg-transparent border-none outline-none text-start font-medium resize-none"
                       rows="5" // Adjust rows as needed
                       style={{ whiteSpace: 'pre-line' }} // This helps in formatting the text correctly
                      />
                      </td>

                <td className={`p-2 border ${isFieldMissing(property.data.area) ? '' : ''}`}>
                  <textarea
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.description) ? '' : ''}`}>
                  <textarea
                    rows="5"
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.owner_name) ? '' : ''}`}>
                  <textarea
                    rows="2"
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.number) ? '' : ''}`}>
                  <input
                    type="text"
                    value={property.data.number || ''}
                    onChange={(e) => handleFieldChange(property._id, 'number', e.target.value)}
                    className="w-24 text-center bg-transparent border-none outline-none font-medium"
                  />
                </td>
                <td className="p-2 border">
                  <select className="font-medium"
                ...
                <td className={`p-2 border ${isFieldMissing(property.data.remark) ? '' : ''}`}>
                  <input
                    rows="5"
